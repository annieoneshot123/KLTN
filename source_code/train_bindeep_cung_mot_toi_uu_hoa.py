...
csv_file = "/home/hai20521281/Downloads/bindeep/labels_cung_mot_toi_uu_hoa.csv"
...
model = tf.keras.models.Sequential([
    tf.keras.layers.LSTM(10, input_shape=(2, 1024), return_sequences=True),  # LSTM layer with fewer units
    tf.keras.layers.Conv1D(128, 1, activation='relu'),  # Convolutional layer with fewer filters and ReLU activation
    tf.keras.layers.GlobalAveragePooling1D(),  # Global average pooling instead of max pooling
    tf.keras.layers.Dense(2, activation='softmax')  # Output layer
])

# Compile the model with the same optimizer but a different compiler
...
# Save the model
model.save('model_bindeep_cung_toi_uu_hoa.h5')
