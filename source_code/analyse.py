...
def parse_bb_to_strand(block):
    ...
    while len(uncovered) > 0:
        ...
        for i in range(last - 1, -1, -1):
            ...
            # Thực hiện Constant Folding
            if needed == True and isinstance(block[i], pyvex.IRStmt.IMark) and isinstance(block[i].expr, pyvex.expr.Const):
            ...
            # Thực hiện Inlining
            elif needed == True and isinstance(block[i], pyvex.IRStmt.IMark) and isinstance(block[i].expr, pyvex.expr.BinOp) and block[i].expr.op in [pyvex.IRExpr.Op.Add, pyvex.IRExpr.Op.Sub, pyvex.IRExpr.Op.Mul, pyvex.IRExpr.Op.UDiv]:
                # Thay thế biểu thức BinOp bằng kết quả hằng số của nó
                ...
                # Tiếp tục phân tích strand bình thường
                if needed == True:
                    strand.append(block[i])
                    used = union(reference(block[i]), used)
                    try:
